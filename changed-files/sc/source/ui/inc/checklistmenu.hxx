...
struct ScCheckListMember
{
    enum DatePartType
    {
        ...
        MONTH,
        DAY,
        TIME, // leaf for datetime values (time-of-day below the day node)
    };
...
private:
    ...
    // For Dates: maps a node's full path (ancestor names joined, see lcl_DatePath)
    // to its index in maMembers, for every year, month and day node. Used to fix
    // up parent pointers while searching, and to find the day member when a
    // datetime value has to convert a date-only day leaf into an inner node.
    std::map<OUString, size_t>    maDateMemberMap;
